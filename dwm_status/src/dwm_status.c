...
size_t print_bat(char* out, size_t out_size)
{
	int	bat;
	size_t	size;

	size = sizeof(bat);
	if (sysctlbyname("hw.acpi.battery.life", &bat, &size, NULL, 0) < 0)
	{
		// colud not get total memory
		return 0;
	}
	size_t bytes_written = snprintf(out, out_size, "[ BAT %d%% ]", bat);
	return bytes_written;
}
...
int main()
{
	...
	while (!done)
	{
		...
		// battery
		offset += print_bat(status + offset, status_size - offset);
